window.VIVA_SYSTEM = (function () {
  ...
  function init() {
    ...
    style.innerHTML = `
      ...
      .glob-vclose { background: none; border: none; color: #9ca3af; font-size: 20px; cursor: pointer; }
      .glob-vopt { width: 100%; text-align: left; padding: 12px 16px; margin-bottom: 8px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: #fff; cursor: pointer; }
      .glob-big-btn { width: 100%; padding: 14px; background: #3b82f6; color: #fff; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; }
    `;
  ...
  function answer(c) {
    ...
    q.opts.forEach(function (_, i) {
      ...
      b.disabled = true;
      if (i === q.ans) b.style.color = '#4ade80';
      else if (i === c) b.style.color = '#f87171';
    });
  ...
  function renderV() {
    ...
    b.innerHTML =
      ...
      '<div style="font-size:16px;font-weight:600;margin-bottom:16px;color:#fff">' + q.q + '</div>' +
      '<div style="display:flex;flex-direction:column;gap:8px">' +
      q.opts.map(function (o, i) {
        return '<button class="glob-vopt" id="glob-vo' + i + '" onclick="window.VIVA_SYSTEM.answer(' + i + ')">' + o + '</button>';
      }).join('') +
      '</div>' +
      '<div id="glob-vexp" style="display:none;margin-top:12px;padding:10px;background:rgba(255,255,255,0.05);border-radius:6px;color:#ddd;"></div>' +
      '<button id="glob-vnext" class="glob-big-btn" style="display:none;margin-top:16px" onclick="window.VIVA_SYSTEM.next()">Next →</button>';
  }
